...
{
    public partial class FormConfeitaria : Form
    {
        ...
        public FormConfeitaria()
        {
            ...
            Listar(listaProdutos.GetListConfeitaria());
        }
        ...
        private void btnAddCarrinho_Click_1(object sender, EventArgs e)
        {
            ...
            {
                foreach (var produto in listaProdutos.GetListConfeitaria())
                {
                    if (produto.Id == Convercao.ConvertInt(tBoxID.Text))
                    {
                        if (listaCarrinho.lista.Contains(produto))
                        {
                            produto.Quantidade = produto.Quantidade + Convercao.ConvertInt(tBoxQuantidade.Text);
